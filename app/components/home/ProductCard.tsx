  ...
  return (
    <Link href={`/${locale}/catalogue/${product.id}`}>
      <article className={`group relative cursor-pointer overflow-hidden rounded-3xl transition-all duration-500 ${
        isCompact
          ? "bg-white ring-1 ring-[#e8e4dc] hover:ring-chocolate/30 hover:shadow-xl hover:shadow-chocolate/5"
          : "bg-white ring-1 ring-[#e8e4dc] hover:ring-chocolate/30 hover:shadow-xl hover:shadow-chocolate/5"
      }`}>
        <div className={`relative overflow-hidden ${isCompact ? "aspect-[16/10]" : "aspect-[4/3]"}`}>
          ...
          <div className="absolute bottom-4 left-4 right-4 flex justify-center opacity-0 translate-y-4 transition-all duration-500 group-hover:opacity-100 group-hover:translate-y-0">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/95 px-5 py-2.5 text-[12px] font-medium text-chocolate shadow-lg backdrop-blur-sm">
        ...
        <div className="p-5">
          <div className="mb-3 flex items-start justify-between gap-2">
            <h3 className="text-[15px] font-semibold text-[#1a1a1a] transition-colors duration-300 group-hover:text-chocolate">
              {product.name}
            </h3>
            {product.brand?.name && (
          ...
          <div className="flex items-end justify-between">
            <div className="flex flex-col gap-1">
              <span className="text-[18px] font-semibold text-chocolate">
